...
def validate_dining(slots):
    ...
    if phone_number is not None:
        if not is_number(phone_number) or len(phone_number) != 10:
            return build_validation_result(False, 'PhoneNumber',
                                           'Sorry. Invalid phone number entered, please enter your phone number with 10 digits')
...
def diningsuggestions_intent(intent_request):
    ...
    phone_number = try_ex(lambda: intent_request['currentIntent']['slots']['PhoneNumber'])
