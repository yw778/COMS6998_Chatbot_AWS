...
user2art = sc.textFile("spark_data/user_artist_data.txt").map(lambda x:[int(k) for k in x.split()])
...
art2name = sc.textFile("spark_data/artist_data.txt").map(safe_name).filter(lambda x: x is not None)
...
def safe_alias(x):
    ...
    if len(data) == 1:
        ...
        try:
            ...
            return None
art_alias = sc.textFile("spark_data/artist_alias.txt").map(safe_alias).filter(lambda x: x is not None)
